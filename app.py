...
def load_data(filename, encoding='latin-1'):
...
def preprocess_text(text, kamus_alay_dict, abusive_list):
    ...
    cleaned_words = [word for word in cleaned_words if word not in ["pengguna", "rt", "user"]]
...
def root():
    return "API untuk Data Cleansing..."
...
@swag_from("docs/reset_data_table.yml", methods=['DELETE'])
def remove_all():
    conn = sqlite3.connect("tweets.db")
    cursor = conn.cursor()
